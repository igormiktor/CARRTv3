...
namespace Lcd
{
    ...
    {
        kOct = 8,     // Produce an octal representation of integers 
                      // (e.g., 11 is output as 013)
        kDec = 10,    // Produce a decimal representation of integers 
                      // (e.g., 11 is output as 11
        kHex = 16     // Produce a hexadecimal representation of integers
                      // (e.g., 11 is output as 0x0b)
    };
    ...
    enum Buttons
    {
        kButton_Select = 0x01,   // the Select button
        kButton_Right  = 0x02,   // the Right button 
        kButton_Down   = 0x04,   // the Down button
        kButton_Up     = 0x08,   // the Up button
        kButton_Left   = 0x10    // the Left button
    };
    ...
    /*!
     * \brief Initialize the Lcd object.  This must be called before using
     * the Lcd, or calling any of the other member functions.  The I2C
     * system must be initialized before calling this function (by calling
     * I2cMaster::start() from I2cMaster.h).
     *
    ...
    /*!
     * \brief Set the backlight to a given color.  Set a black-and-white LCD
     * display to White if you want to have a backlight.
     *
     * \arg \c color the color to set the backlight.  Pass one of
     * kBacklight_Red, kBacklight_Yellow, kBacklight_Green, kBacklight_Teal,
     * kBacklight_Blue, kBacklight_Violet, or kBacklight_White.
     */
     ...
     *
     * \returns a byte with flags set corresponding to the buttons that are
     * depressed.  You must "and" the return value with kButton_Right,
     * kButton_Left, kButton_Down, kButton_Up, or kButton_Select to determine
     * which buttons have been pressed.
     */
    int readButtons();

    /*!
     * \brief Write a single character to the LCD at the current cursor
     * location.
    ...
    /*!
     * \brief Write a given number of characters from a buffer to the LCD
    ...
    /*!
     * \brief Write a given number of bytes from a buffer to the LCD
     * at the current cursor location.
     * This implements the pure virtual function
     * Writer::write( const std::uint8_t* buffer, size_t size ).
     *
    ...
    template<typename T>
    void print( T x, Lcd::IntegerOutputBase base )
    {
        std::stringstream tmp;
        switch ( base )
        {
