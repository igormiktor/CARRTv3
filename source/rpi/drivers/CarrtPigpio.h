...
/*
    Prefer the pigpio function names, so map these to the pigpiod
    function names if pigpiod is being usedCarrtPigpio.h
...
extern "C"
{
    ...
    inline void gpioTerminate() { pigpio_stop( gPigpioId ); }
    ...
    // No pigpiod equivalent
    // int gpioSetAlertFunc(unsigned user_gpio, gpioAlertFunc_t f);

    // No pigpiod equivalent
    // int gpioSetAlertFuncEx( unsigned user_gpio, gpioAlertFuncEx_t f, void
    // *userdata );
    ...
    inline int gpioNotifyOpen( void ) { return notify_open( gPigpioId ); }
    ...
    inline int gpioWaveClear( void ) { return wave_clear( gPigpioId ); }

    inline int gpioWaveAddNew( void ) { return wave_add_new( gPigpioId ); }
    ...
    inline int gpioWaveAddSerial( unsigned user_gpio, unsigned baud,
                                  unsigned data_bits, unsigned stop_bits,
                                  unsigned offset, unsigned numBytes,
                                  char* str )
    {
        return wave_add_serial( gPigpioId, user_gpio, baud, data_bits,
                                stop_bits, offset, numBytes, str );
    }

    inline int gpioWaveCreate( void ) { return wave_create( gPigpioId ); }

    // No exact pigpiod equivalent for this
    ...
    inline int gpioWaveTxAt( void ) { return wave_tx_at( gPigpioId ); }

    inline int gpioWaveTxBusy( void ) { return wave_tx_busy( gPigpioId ); }

    inline int gpioWaveTxStop( void ) { return wave_tx_stop( gPigpioId ); }
    ...
    inline int gpioWaveGetCbs( void ) { return wave_get_cbs( gPigpioId ); }
    ...
    inline int gpioSerialReadOpen( unsigned user_gpio, unsigned baud,
                                   unsigned data_bits )
    {
    ...
    inline int i2cWriteByteData( unsigned handle, unsigned i2cReg,
                                 unsigned bVal )
    {
        return i2c_write_byte_data( gPigpioId, handle, i2cReg, bVal );
    }

    inline int i2cWriteWordData( unsigned handle, unsigned i2cReg,
                                 unsigned wVal )
    {
    ...
    inline int i2cWriteBlockData( unsigned handle, unsigned i2cReg, char* buf,
                                  unsigned count )
    {
    ...
    inline int i2cBlockProcessCall( unsigned handle, unsigned i2cReg, char* buf,
                                    unsigned count )
    {
        return i2c_block_process_call( gPigpioId, handle, i2cReg, buf, count );
    }

    inline int i2cReadI2CBlockData( unsigned handle, unsigned i2cReg, char* buf,
                                    unsigned count )
    {
        return i2c_read_i2c_block_data( gPigpioId, handle, i2cReg, buf, count );
    }

    inline int i2cWriteI2CBlockData( unsigned handle, unsigned i2cReg,
                                     char* buf, unsigned count )
    {
        return i2c_write_i2c_block_data( gPigpioId, handle, i2cReg, buf,
                                         count );
    }
    ...
    inline int bbI2CZip( unsigned SDA, char* inBuf, unsigned inLen,
                         char* outBuf, unsigned outLen )
    {
    ...
    inline int bbSPIOpen( unsigned CS, unsigned MISO, unsigned MOSI,
                          unsigned SCLK, unsigned baud, unsigned spiFlags )
    {
    ...
    inline int bbSPIXfer( unsigned CS, char* inBuf, char* outBuf,
                          unsigned count )
    {
    ...
    inline int spiXfer( unsigned handle, char* txBuf, char* rxBuf,
                        unsigned count )
    {
    ...
    inline int gpioTrigger( unsigned user_gpio, unsigned pulseLen,
                            unsigned level )
    {
    ...
    inline int gpioNoiseFilter( unsigned user_gpio, unsigned steady,
                                unsigned active )
    {
    ...
    // Not defined in pigpiod
    // inline int gpioSetGetSamplesFuncEx( gpioGetSamplesFuncEx_t f, 
    // uint32_t bits, void *userdata );

    // Not defined in pigpiod
    // int gpioSetTimerFunc( unsigned timer, unsigned millis, 
    // gpioTimerFunc_t f );

    // Not defined in pigpiod
    // int gpioSetTimerFuncEx( unsigned timer, unsigned millis,
    ...
    inline void gpioStopThread( pthread_t* pth ) { stop_thread( pth ); }
    ...
    inline int gpioRunScript( unsigned script_id, unsigned numPar,
                              uint32_t* param )
    {
        return run_script( gPigpioId, script_id, numPar, param );
    }

    inline int gpioUpdateScript( unsigned script_id, unsigned numPar,
                                 uint32_t* param )
    {
    ...
    inline int gpioHardwarePWM( unsigned gpio, unsigned PWMfreq,
                                unsigned PWMduty )
    {
    ...
    inline uint32_t gpioTick( void ) { return get_current_tick( gPigpioId ); }
    ...
    // Not defined in pigpiod
    // int gpioCfgClock( unsigned cfgMicros, unsigned cfgPeripheral, unsigned
    // cfgSource ;
    ...
    // Not defined in pigpiod
    // int gpioCfgDMAchannels(unsigned primaryChannel, unsigned
    // secondaryChannel);
    ...
    // Not defined in pigpiod
    // int gpioCustom2(unsigned arg1, char *argx, unsigned argc, char *retBuf,
    // unsigned retMax);

    // Not defined in pigpiod
    // int rawWaveAddSPI(rawSPI_t *spi, unsigned offset, unsigned spiSS, char
    // *buf, unsigned spiTxBits,
