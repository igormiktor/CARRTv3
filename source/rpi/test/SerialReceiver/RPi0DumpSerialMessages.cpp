/*
    RPi0DumpSerialMessages.cpp - Special version of Serial Messages that
    simply dumps incoming messages to std::cout.
    ...
    : SerialMessage( id ), mContent( MsgId::kUnknownMessage ), mNeedsAction{ false }
{
    ...
    {
        throw CarrtError( makeRpi0ErrorId( kRPi0SerialMessageError, 1,
...
void PingMsg::takeAction( EventManager& events, SerialLink& link )
{
    ...
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got PingMsg from Pico, sent PingReplyMsg" );
...
void PingReplyMsg::takeAction( EventManager&, SerialLink& link )
{
    ...
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got ping reply from Pico" );
    ...
    : SerialMessage( id ), mContent( MsgId::kPicoReady ), mNeedsAction{ false }
{
    ...
    {
        throw CarrtError(
            makeRpi0ErrorId( kRPi0SerialMessageError, 1, std::to_underlying( MsgId::kPicoReady ) ),
            "Id mismatch at creation" );
    }
    // Note that it doesn't need action until loaded with data
}
...
void PicoReadyMsg::readIn( SerialLink& link )
{
    ...
    debugCond2cout<kDebugSerialMsgs>( "Got PicoReadyMsg", getIdNum(),
...
void PicoReadyMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got PicoReadyMsg", getIdNum(), std::get<0>( mContent.mMsg ) );
    }
}
...
void PicoNavStatusUpdateMsg::readIn( SerialLink& link )
{
    ...
    debugCond2cout<kDebugSerialMsgs>( "Got PicoNavStatusUpdateMsg", getIdNum(),
                                      std::get<0>( mContent.mMsg ),
...
void PicoNavStatusUpdateMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 sending PicoNavStatusUpdateMsg", getIdNum(),
                 std::get<0>( mContent.mMsg ), static_cast<int>( std::get<1>( mContent.mMsg ) ),
                 static_cast<int>( std::get<2>( mContent.mMsg ) ),
                 static_cast<int>( std::get<3>( mContent.mMsg ) ),
                 static_cast<int>( std::get<4>( mContent.mMsg ) ) );
}
...
void PicoNavStatusUpdateMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got PicoNavStatusUpdateMsg", getIdNum(), std::get<0>( mContent.mMsg ),
                     static_cast<int>( std::get<1>( mContent.mMsg ) ),
                     static_cast<int>( std::get<2>( mContent.mMsg ) ),
                     static_cast<int>( std::get<3>( mContent.mMsg ) ),
                     static_cast<int>( std::get<4>( mContent.mMsg ) ) );
...
void PicoSaysStopMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got PicoSayStopMsg", getIdNum() );
    }
}
...
void MsgControlMsg::readIn( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 got MsgControlMsg", getIdNum(),
...
void ResetPicoMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got ResetPicoMsg", getIdNum() );
    }
}
...
void TimerEventMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got TimerEventMsg", getIdNum(),
                     static_cast<int>( std::get<0>( mContent.mMsg ) ), std::get<1>( mContent.mMsg ),
                     std::get<2>( mContent.mMsg ) );
    }
}
...
void TimerControlMsg::readIn( SerialLink& link )
{
    ...
    output2cout( "Error: Rpi0 got TimerControlMsg", getIdNum(),
                 static_cast<int>( std::get<0>( mContent.mMsg ) ) );
}
...
void TimerControlMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( " sendQtrSecTimerMsgs",
                 static_cast<bool>( std::get<0>( mContent.mMsg ) & kQtrSecTimerMsgMask ) );
    output2cout( " sendQ1SecTimerMsgs",
                 static_cast<bool>( std::get<0>( mContent.mMsg ) & k1SecTimerMsgMask ) );
    output2cout( " sendQ8SecTimerMsgs",
                 static_cast<bool>( std::get<0>( mContent.mMsg ) & k8SecTimerMsgMask ) );
}
...
void CalibrationInfoUpdateMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        output2cout( "Got CalibrationInfoUpdateMsg", getIdNum(),
                     static_cast<int>( std::get<0>( mContent.mMsg ) ),
                     static_cast<int>( std::get<1>( mContent.mMsg ) ),
                     static_cast<int>( std::get<2>( mContent.mMsg ) ),
                     static_cast<int>( std::get<3>( mContent.mMsg ) ) );
...
void SetAutoCalibrateMsg::readIn( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 got SetAutoCalibrateMsg", getIdNum(),
...
void SetAutoCalibrateMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "Sent Pico SetAutoCalibrateMsg set to",
                 static_cast<bool>( std::get<0>( mContent.mMsg ) ) );
}
...
void NavUpdateMsg::readIn( SerialLink& link )
{
    ...
    debugCond2cout<kDebugSerialMsgs>( "Got NavUpdateMsg", getIdNum(), std::get<0>( mContent.mMsg ),
...
void NavUpdateMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 sending NavUpdateMsg", getIdNum(), std::get<0>( mContent.mMsg ),
                 std::get<1>( mContent.mMsg ) );
}
...
void NavUpdateMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got NavUpdateMsg", getIdNum(), std::get<0>( mContent.mMsg ),
                     std::get<1>( mContent.mMsg ) );
    }
}
...
void DrivingStatusUpdateMsg::readIn( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 got DrivingStatusUpdateMsg", getIdNum(),
                 static_cast<int>( std::get<0>( mContent.mMsg ) ) );
}
...
void DrivingStatusUpdateMsg::sendOut( SerialLink& link )
{
    ...
    debugCond2cout<kDebugSerialMsgs>( "RPi0 sent DrivingStatusUpdateMsg", getIdNum(),
                                      static_cast<int>( std::get<0>( mContent.mMsg ) ) );
}
...
void EncoderUpdateMsg::readIn( SerialLink& link )
{
    ...
    debugCond2cout<kDebugSerialMsgs>( "RPi0 got EncoderUpdateMsg", std::get<0>( mContent.mMsg ),
                                      std::get<1>( mContent.mMsg ), std::get<2>( mContent.mMsg ) );
}
...
void EncoderUpdateMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 sends EncoderUpdateMsg", std::get<0>( mContent.mMsg ),
                 std::get<1>( mContent.mMsg ), std::get<2>( mContent.mMsg ) );
}
...
void EncoderUpdateMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got EncoderUpdateMsg", getIdNum(),
                     ( static_cast<bool>( std::get<0>( mContent.mMsg ) ) ? "Right" : "Left" ),
                     std::get<1>( mContent.mMsg ), std::get<2>( mContent.mMsg ) );
    }
}
...
void EncoderUpdateControlMsg::readIn( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 got EncoderUpdateControlMsg", getIdNum(),
                 static_cast<bool>( std::get<0>( mContent.mMsg ) ) );
}
...
void EncoderUpdateControlMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "RPi0 sent EncoderUpdateControlMsg with",
                 static_cast<bool>( std::get<0>( mContent.mMsg ) ) );
}
...
void BatteryLevelRequestMsg::readIn( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 got BatteryLevelRequestMsg",
                 static_cast<int>( std::get<0>( mContent.mMsg ) ) );
}
...
void BatteryLevelRequestMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "RPi0 sent BatteryLevelRequestMsg",
                 static_cast<int>( std::get<0>( mContent.mMsg ) ) );
}
...
void BatteryLevelUpdateMsg::readIn( SerialLink& link )
{
    ...
    debugCond2cout<kDebugSerialMsgs>( "RPi0 got BatteryLevelUpdateMsg", getIdNum(),
                                      static_cast<int>( std::get<0>( mContent.mMsg ) ),
                                      std::get<1>( mContent.mMsg ) );
}
...
void BatteryLevelUpdateMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 sending BatteryLevelUpdateMsg", getIdNum(),
                 static_cast<int>( std::get<0>( mContent.mMsg ) ), std::get<1>( mContent.mMsg ) );
}
...
void BatteryLevelUpdateMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got BatteryLevelUpdateMsg", getIdNum(),
                     ( static_cast<bool>( std::get<0>( mContent.mMsg ) ) ? "Motor" : "IC" ),
                     std::get<1>( mContent.mMsg ) );
    }
}
...
void ErrorReportMsg::readIn( SerialLink& link )
{
    ...
    debugCond2cout<kDebugSerialMsgs>( "RPi0 got ErrorReportMsg", getIdNum(),
                                      static_cast<bool>( std::get<0>( mContent.mMsg ) ),
                                      std::get<1>( mContent.mMsg ), std::get<2>( mContent.mMsg ) );
}
...
void ErrorReportMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 sending ErrorReportMsg", getIdNum(),
...
void ErrorReportMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got ErrorReportMsg", getIdNum(),
...
void TestPicoErrorRptMsg::readIn( SerialLink& link )
{
    ...
    output2cout( "Error: RPi0 got TestPicoErrorRptMsg", getIdNum(),
                 static_cast<bool>( std::get<0>( mContent.mMsg ) ), std::get<1>( mContent.mMsg ) );
}
...
void TestPicoErrorRptMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "RPi0 SentTestPicoErrorRptMsg requesting",
                 ( static_cast<bool>( std::get<0>( mContent.mMsg ) ) ? "Fatal" : "Not Fatal" ),
                 "error code", std::get<1>( mContent.mMsg ) );
}
...
void TestPicoMessagesMsg::readIn( SerialLink& link )
{
    ...
    output2cout( "RPi0 got TestPicoMessagesMsg", getIdNum(),
                 static_cast<int>( std::get<0>( mContent.mMsg ) ) );
}
...
void TestPicoMessagesMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "RPi0 sent TestPicoMessagesMsg", getIdNum(),
                 static_cast<int>( std::get<0>( mContent.mMsg ) ) );
}
...
PicoReceivedTestMsg::PicoReceivedTestMsg( std::uint8_t msgIdToSendBack ) noexcept
    : SerialMessage( MsgId::kPicoReceivedTestMsg ),
      mContent( MsgId::kPicoReceivedTestMsg, std::make_tuple( msgIdToSendBack ) ),
      mNeedsAction{ true }
{}
    ...
    : SerialMessage( id ), mContent( MsgId::kPicoReceivedTestMsg ), mNeedsAction{ false }
{
    ...
    {
        throw CarrtError( makeRpi0ErrorId( kPicoSerialMessageError, 1,
...
{
    mContent.readIn( link );
    debugCond2cout<kDebugSerialMsgs>( "RPi0 received PicoReceivedTestMsg", getIdNum(),
                                      static_cast<int>( std::get<0>( mContent.mMsg ) ) );
...
{
    // RPi0 never sends this out
    output2cout( "ERROR: RPi0 sending PicoReceivedTestMsg", getIdNum(),
                 static_cast<int>( std::get<0>( mContent.mMsg ) ) );
}
...
void PicoReceivedTestMsg::takeAction( EventManager& evt, SerialLink& link )
{
    ...
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got Pico confirmation of RPi0 request for MsgId",
                     static_cast<int>( std::get<0>( mContent.mMsg ) ), "that Pico doesn't send" );
    }
}
...
void DebugLinkMsg::readIn( SerialLink& link )
{
    ...
    debugCond2cout<kDebugSerialMsgs>( "RPi0 got DebugLinkMsg", std::get<0>( mContent.mMsg ),
                                      static_cast<int>( std::get<1>( mContent.mMsg ) ),
...
void DebugLinkMsg::sendOut( SerialLink& link )
{
    ...
    output2cout( "RPi0 sent DebugLinkMsg", std::get<0>( mContent.mMsg ),
...
void DebugLinkMsg::takeAction( EventManager&, SerialLink& link )
{
    if ( mNeedsAction )
    {
        ...
        output2cout( "Got DebugLinkMsg", getIdNum(), std::get<0>( mContent.mMsg ),
                     static_cast<int>( std::get<1>( mContent.mMsg ) ), std::get<2>( mContent.mMsg ),
                     std::get<3>( mContent.mMsg ) );
